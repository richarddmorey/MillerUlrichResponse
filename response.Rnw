...
Consider MU's statement 4' for a $CI_{95\%}$ of $[.1, .4]$ (``If the current sample is one of the 95\% of all samples with relatively small values of $|\bar{X}-\mu|/S$, then $\mu$ lies in the interval $0.1-0.4$'', p. 7). Let ``relatively small'' mean that $|\bar{X}-\mu|/S<c$ for some positive value $c$. Then a bit of algebra yields 
\[
...
s = diff(ci.survey)/2 *sqrt(N)/tstar_95

ci_95 = xbar + c(-1,1)*tstar_95*s/sqrt(N)
ci_91 = xbar + c(-1,1)*tstar_91*s/sqrt(N)
ci_99 = xbar + c(-1,1)*tstar_99*s/sqrt(N)
...
par(mar=c(5,0,0,0), mfrow = c(1,2))
plot(1,1, ty = 'n', ylim = c(0,1), xlim = c(-1,1), axes=FALSE, ylab = "", xlab = "Mean")

arrows(ci_95[1], 3/5,ci_95[2], 3/5, code = 3, angle=90, len = .1)
arrows(ci_91[1], 2/5,ci_91[2], 2/5, code = 3, angle=90, len = .1)
arrows(ci_99[1], 4/5,ci_99[2], 4/5, code = 3, angle=90, len = .1)
arrows(ci_91[1], 1/5,ci_91[2], 1/5, code = 3, angle=90, len = .1)


text(-1,4/5,"CP-D:", adj=0)
text(-1,3/5,"CP-A:", adj=0)
text(-1,2/5,"CP-E:", adj=0)
text(-1,1/5,"CP-C:", adj=0)

axis(1)

mtext("A", 3, adj=.95,-1.5, cex=1.5)
box()

#abline(v = 0, col="gray", lty=1)
abline(v = xbar, col="gray", lty=2)

points(rep(xbar,4), (1:4)/5, pch=21, col="black", bg="lightblue")

par(mar=c(5,0,0,0))
plot(1,1, ty = 'n', ylim = c(0,1), xlim = c(0,1), axes=FALSE, ylab = "", xlab = "")

xpts = seq(.1,.70,len=seq.len)
xwidth=diff(xpts)[1] * 1.1

points(xpts, rep(3/5,seq.len), pch="X")
text(c(xpts[1] - xwidth, xpts[seq.len] + xwidth), rep(3/5,2), expression(cdots))

whichCI = sample(1:2,seq.len, replace =TRUE)

points(xpts, rep(2/5,seq.len), pch=c("Y","Z")[whichCI]) 
text(c(xpts[1] - xwidth, xpts[seq.len] + xwidth), rep(2/5,2), expression(cdots))

points(xpts, rep(4/5,seq.len), pch=c("Z","Y")[whichCI])
text(c(xpts[1] - xwidth, xpts[seq.len] + xwidth), rep(4/5,2), expression(cdots))

points(xpts, rep(1/5,seq.len), pch="Z")
text(c(xpts[1] - xwidth, xpts[seq.len] + xwidth), rep(1/5,2), expression(cdots))



text(1,4/5,"Pr(in)=95%", adj=1)
text(1,2/5,"Pr(in)=95%",adj=1)
text(1,3/5,"Pr(in)=95%", adj=1)
text(1,1/5,"Pr(in)=91%", adj=1)

first.z = which(whichCI==2)[1]

ymarg = .05

rect(xpts[first.z] - xwidth/2, 1/5 - ymarg, xpts[first.z] + xwidth/2, 2/5 + ymarg, border="red")

box()
mtext("B", 3, adj=.95,-1.5, cex=1.5)
